...
function isRavenDisabled() {
  try {
    ...
    if (typeof window.disableRaven !== "undefined" && window.disableRaven)
      return true;
    return false;
  } catch (ex) {
    return false;
...
window.onerror = function (msg, url, line, column, err) {
  ...
  document.querySelector(".app .error .error-description").innerHTML =
    'Please try reloading the page or using a different browser (Chrome or Firefox), and if the error still persists, <a href="https://github.com/pgaskin/ePubViewer/issues">report an issue</a>.';
  ...
  try {
    if (!isRavenDisabled()) console.error(err);
  } catch (err) {}
};
...
let App = function (el) {
  ...
  this.qsa(".tab-list .item").forEach((el) =>
    el.addEventListener("click", this.onTabClick.bind(this, el.dataset.tab)),
  );
  this.qs(".sidebar .search-bar .search-box").addEventListener(
    "keydown",
    (event) => {
      if (event.keyCode == 13)
        this.qs(".sidebar .search-bar .search-button").click();
    },
  );
  this.qs(".sidebar .search-bar .search-button").addEventListener(
    "click",
  ...
  this.qsa(".chips[data-chips]").forEach((el) => {
    Array.from(el.querySelectorAll(".chip[data-value]")).forEach((cel) =>
  ...
  try {
    ...
    this.qs(".bar .loc").addEventListener("click", (event) => {
      try {
        let answer = prompt(
          `Location to go to (up to ${this.state.book.locations.length()})?`,
          this.state.rendition.currentLocation().start.location,
        );
        if (!answer) return;
        answer = answer.trim();
        if (answer == "") return;
        ...
        if (isNaN(parsed) || parsed < 0)
          throw new Error("Invalid location: not a positive integer");
        if (parsed > this.state.book.locations.length())
...
App.prototype.doBook = function (url, opts) {
  ...
  this.state.rendition.on(
    "displayed",
    this.onRenditionDisplayedTouchSwipe.bind(this),
  );
  this.state.rendition.on(
    "relocated",
    this.onRenditionRelocatedUpdateIndicators.bind(this),
  );
  this.state.rendition.on(
    "relocated",
    this.onRenditionRelocatedSavePos.bind(this),
  );
  this.state.rendition.on(
    "started",
    this.onRenditionStartedRestorePos.bind(this),
  );
  this.state.rendition.on(
    "displayError",
    this.fatal.bind(this, "error rendering book"),
  );

  this.state.rendition.display();

  if (this.state.dictInterval) window.clearInterval(this.state.dictInterval);
  this.state.dictInterval = window.setInterval(
    this.checkDictionary.bind(this),
    50,
  );
...
App.prototype.loadSettingsFromStorage = function () {
  ["theme", "font", "font-size", "line-spacing", "margin", "progress"].forEach(
    (container) => this.restoreChipActive(container),
  );
};
...
App.prototype.setDefaultChipActive = function (container) {
  let el = this.qs(`.chips[data-chips='${container}']`).querySelector(
    ".chip[data-default]",
  );
...
App.prototype.setChipActive = function (container, value) {
  Array.from(
    this.qs(`.chips[data-chips='${container}']`).querySelectorAll(
      ".chip[data-value]",
    ),
  ).forEach((el) => {
...
App.prototype.getChipActive = function (container) {
  let el = this.qs(`.chips[data-chips='${container}']`).querySelector(
    ".chip.active[data-value]",
  );
  if (!el)
    return this.qs(`.chips[data-chips='${container}']`).querySelector(
      ".chip[data-default]",
    );
  return el.dataset.value;
};
...
App.prototype.fatal = function (msg, err, usersFault) {
  ...
  document.querySelector(".app .error .error-description").innerHTML =
    usersFault
      ? ""
      : 'Please try reloading the page or using a different browser, and if the error still persists, <a href="https://github.com/pgaskin/ePubViewer/issues">report an issue</a>.';
  document.querySelector(".app .error .error-info").innerHTML =
    msg + ": " + err.toString();
  document.querySelector(".app .error .error-dump").innerHTML = JSON.stringify({
    error: err.toString(),
    stack: err.stack,
...
App.prototype.doReset = function () {
  ...
  this.qs(".book").innerHTML =
    '<div class="empty-wrapper"><div class="empty"><div class="app-name">ePubViewer</div><div class="message"><a href="javascript:ePubViewer.doOpenBook();" class="big-button">Open a Book</a></div></div></div>';
...
App.prototype.getNavItem = function (loc, ignoreHash) {
  return (
    ...
    })(this.state.book.navigation.toc).filter((item) =>
      ignoreHash
        ? this.state.book.canonical(item.href).split("#")[0] ==
          this.state.book.canonical(loc.start.href).split("#")[0]
        : this.state.book.canonical(item.href) ==
          this.state.book.canonical(loc.start.href),
    )[0] || null
  );
};
...
App.prototype.onRenditionRelocated = function (event) {
  ...
  try {
    ...
    this.qsa(".toc-list .item").forEach((el) =>
      el.classList[
        navItem && el.dataset.href == navItem.href ? "add" : "remove"
      ]("active"),
    );
  } catch (err) {
...
App.prototype.onBookMetadataLoaded = function (metadata) {
  ...
  if (!metadata.series || metadata.series.trim() == "")
    this.qs(".info .series-info").classList.add("hidden");
  ...
  if (sanitizeHtml)
    this.qs(".info .description").innerHTML = sanitizeHtml(
  ...
  try {
    if (this.getChipActive("progress") == "bar") {
      ...
      range.addEventListener(
        ...
        () =>
          this.state.rendition.display(
            this.state.book.locations.cfiFromLocation(range.value),
          ),
        false,
      );
      ...
      for (let i = 0, last = -1; i < this.state.book.locations.length(); i++) {
        try {
          let parsed = new ePub.CFI().parse(
            this.state.book.locations.cfiFromLocation(i),
          );
    ...
    if (this.getChipActive("progress") == "none") {
      ...
      this.getChipActive("progress") == "location" &&
      event.start.location > 0
    ) {
      stxt = `Loc ${event.start.location}/${this.state.book.locations.length()}`;
    } else if (this.getChipActive("progress") == "chapter") {
      let navItem =
        this.getNavItem(event, false) || this.getNavItem(event, true);
      stxt = navItem
        ? navItem.label.trim()
...
App.prototype.checkDictionary = function () {
  try {
    let selection =
      this.state.rendition.manager &&
      this.state.rendition.manager.getContents().length > 0
        ? this.state.rendition.manager
            .getContents()[0]
            .window.getSelection()
            .toString()
    ...
    if (selection.length < 2 || selection.indexOf(" ") > -1) {
      if (this.state.showDictTimeout)
        window.clearTimeout(this.state.showDictTimeout);
      this.doDictionary(null);
      return;
    }
    this.state.showDictTimeout = window.setTimeout(() => {
      try {
        let newSelection = this.state.rendition.manager
          .getContents()[0]
          .window.getSelection()
          .toString()
          .trim();
...
App.prototype.doDictionary = function (word) {
  ...
  if (!this.qs(".dictionary-wrapper").classList.contains("hidden"))
  ...
  let ldefinitionEl = this.qs(".dictionary").appendChild(
  ...
  fetch(`https://dict.geek1011.net/word/${encodeURIComponent(word)}`)
    .then((resp) => {
    ...
    .then((obj) => {
      ...
      [obj]
        .concat(obj.additional_words || [])
        .concat(obj.referenced_words || [])
        .map((word) => {
          let definitionEl = this.qs(".dictionary").appendChild(
            document.createElement("div"),
          );
          ...
          wordEl.innerText = [word.word]
            .concat(word.alternates || [])
            .join(", ")
            .toLowerCase();

          let meaningsEl = definitionEl.appendChild(
            document.createElement("div"),
          );
          ...
          (word.meanings || []).map((meaning, i) => {
            let meaningEl = meaningsEl.appendChild(
              document.createElement("div"),
            );
            meaningEl.classList.add("meaning");

            let meaningTextEl = meaningEl.appendChild(
              document.createElement("div"),
            );
            ...
            if (meaning.example && meaning.example.trim() != "") {
              let meaningExampleEl = meaningEl.appendChild(
                document.createElement("div"),
              );
          ...
          if (word.credit && word.credit.trim() != "") {
            let creditEl = meaningsEl.appendChild(
              document.createElement("div"),
            );
...
App.prototype.doTab = function (tab) {
  try {
    this.qsa(".tab-list .item").forEach((el) =>
      el.classList[el.dataset.tab == tab ? "add" : "remove"]("active"),
    );
    this.qsa(".tab-container .tab").forEach((el) =>
      el.classList[el.dataset.tab != tab ? "add" : "remove"]("hidden"),
    );
    try {
...
App.prototype.onSearchClick = function (event) {
  this.doSearch(this.qs(".sidebar .search-bar .search-box").value.trim())
    .then((results) => {
      ...
      results.slice(0, 200).forEach((result) => {
        ...
        resultEl.addEventListener(
          "click",
          this.onResultClick.bind(this, result.cfi),
        );
        ...
        resultEl
          .appendChild(this.el("div", "pbar"))
          .appendChild(this.el("div", "pbar-inner")).style.width =
          (
...
try {
  ...
  if (ufn.startsWith("!")) {
    ufn = ufn.replace("!", "");
    document.querySelector(".app button.open").style =
