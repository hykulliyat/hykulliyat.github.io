...
if (typeof ReadableStream !== 'undefined' && ReadableStream.prototype[Symbol.asyncIterator] === undefined) {
...
class EpubViewer {
    #rootEl; #state; #domCache = new Map();
    constructor(el) { this.#rootEl = el; this.#state = {}; this.#init(); }
    static instance = null;
    #init() { setTimeout(() => { this.#setupEvents(); this.applyTheme(); }, 100); }
    #getEl(s) { if (!this.#domCache.has(s)) { const e = this.#rootEl.querySelector(s); if (e) this.#domCache.set(s, e); return e; } return this.#domCache.get(s); }
    #getEls(s) { return Array.from(this.#rootEl.querySelectorAll(s)); }
    ...
    #setupEvents() {
        ...
        if (searchBox) {
            ...
            searchBox.addEventListener("keydown", (e) => {
                Logger.log("Search box keydown:", e.keyCode, "key:", e.key, "isComposing:", isComposing);
            ...
            searchBox.addEventListener("input", (e) => {
                Logger.log("Search box input value:", searchBox.value, "isComposing:", e.isComposing);
            });
        }
    ...
    onSearchClick() {
        const query = this.#getEl(".sidebar .search-bar .search-box")?.value?.trim();
        if (!query) return;
        ...
        this.doSearch(query).then(results => {
            ...
            if (results.length === 0) {
                resultsContainer.innerHTML = "<div class='item'>Sonuç bulunamadı</div>";
                return;
            }
            ...
            results.slice(0, 200).forEach(result => {
                ...
                resultEl.addEventListener("click", (e) => this.onResultClick(result.cfi, e));
                ...
                if (this.#state.book?.locations?.percentageFromCfi) {
                    pbarInner.style.width = (this.#state.book.locations.percentageFromCfi(result.cfi) * 100).toFixed(3) + "%";
                }
    ...
    onLocationClick() {
        try {
            ...
            const current = this.#state.rendition?.currentLocation?.()?.start?.location || 0;
    ...
    updatePageIndicator(event) {
                const locEl = this.#getEl(".bar .loc");
        if (!locEl) { Logger.warn("Loc element not found"); return; }
        ...
        if (current === 0 && total > 0 && event?.start?.cfi) {
            try {
                current = this.#state.book.locations.locationFromCfi(event.start.cfi) || 0;
                Logger.log("Location calculated from CFI:", { cfi: event.start.cfi, current });
            } catch (err) {
                Logger.warn("Failed to calculate location from CFI:", err);
            }
        }
        
        Logger.log("Page indicator update:", { total, current, eventLocation: event?.start?.location, cfi: event?.start?.cfi?.substring(0, 30) });
    ...
    loadSettingsFromStorage() {
        const chips = ["theme", "font", "font-size", "line-spacing", "margin", "progress"];
        chips.forEach(chip => {
    ...
    async doBook(url, opts = { encoding: "epub" }) {
        if (typeof ePub !== "function") {
            ErrorBoundary.handle("Kutuphane hatasi", new Error("ePub.js yuklenemedi"));
            return;
        }
        Logger.log("Loading book", opts);
        const bookEl = document.getElementById('epub-container') || this.#getEl(".book");
        ...
        try {
            ...
            this.#state.book.ready.then(() => {
                ...
                if (!hasSpine) { ErrorBoundary.handle("EPUB hatasi", new Error("Spine bulunamadi")); return; }
                ...
                for (let i = 0; i < this.#state.book.spine.spineItems.length; i++) {
                    ...
                    if (s.href && s.linear !== false) {
                        ...
                        if (hrefLower.includes('nav.') || hrefLower.includes('toc.') || hrefLower === 'nav.xhtml') {
                            continue;
                        }
                        firstValidSection = s;
                ...
                if (!firstValidSection) {
                    ErrorBoundary.handle("EPUB hatasi", new Error("Gecerli bolum bulunamadi"));
                    return;
                }
                ...
                this.#state.rendition = this.#state.book.renderTo(bookEl, { width: "100%", height: "100%" });
                ...
                this.#state.rendition.on("relocated", (location) => {
                    ...
                    if (location?.start?.href) {
                        this.#getEls(".toc-list .item").forEach(el => {
                            el.classList.toggle("active", el.dataset.href === location.start.href);
                        });
                    }
                });
                ...
                this.#state.rendition.display(firstValidSection.href).then(() => { 
                    ...
                    this.#state.rendition.display(fallbackIndex).then(() => {
    ...
    generateLocations(chars = 1600) {
        ...
        if (this.#state.book.locations.length() > 0) {
            Logger.log("Locations already generated:", this.#state.book.locations.length());
            return;
        }
        ...
        Logger.log(`Spine items: ${spineItems.length}, valid: ${validItems.length}`);
        ...
        this.#state.book.locations.generate(chars).then(() => {
            const count = this.#state.book.locations.length();
            ...
            try {
                Storage.set(this.#state.book.key() + "_locations", this.#state.book.locations.save());
            } catch (err) {}
        }).catch(err => {
    ...
    doReset() {
        ...
        this.#getEl(".sidebar .search-results") && (this.#getEl(".sidebar .search-results").innerHTML = "");
    ...
    onBookMetadataLoaded(metadata) {
        ...
        DOM.setText(this.#getEl(".bar .book-author"), metadata.creator?.trim() || "");
        ...
        if (seriesInfo) {
            if (metadata.series && metadata.series.trim() !== "") {
                ...
                DOM.setText(this.#getEl(".info .series-index"), metadata.seriesIndex?.trim() || "");
            } else {
    ...
    onBookCoverLoaded(url) {
        ...
        this.#state.book.archive.createUrl(url).then((blobUrl) => {
            cover.src = blobUrl;
        }).catch(err => {
    ...
    onBookReady() {
        ...
        Logger.log("Buttons found:", { sidebar: !!sidebarBtn, prev: !!prevBtn, next: !!nextBtn });
            ...
            try {
                ...
                if (ext === 'pdf') {
                    if (window.bookReaderApp) {
                        await window.bookReaderApp.loadBookFromBuffer(arrayBuffer, fileName, 'pdf');
                    }
                } else if (ext === 'epub') {
                    await this.doBook(arrayBuffer, { encoding: "binary", name: fileName });
                } else {
                    ErrorBoundary.handle("Gecersiz dosya", new Error("Desteklenmeyen format: " + ext));
                }
            } catch (err) {
...
class PdfViewer {
    ...
    async loadBook(buffer, bookId, startPage = 1) {
        ...
        try {
            ...
            // Clear container
            this.#container.innerHTML = '<div class="pdf-loading">PDF yükleniyor...</div>';
            this.#pagesRendered.clear();
            ...
            pagesWrapper.style.cssText = 'display: flex; flex-direction: column; align-items: center; gap: 20px; width: 100%;';
        ...
        for (let i = 1; i <= this.#totalPages; i++) {
            ...
            pageContainer.style.cssText = 'min-height: 500px; display: flex; justify-content: center; width: 100%;';
        ...
        if (this.#onPositionChange) {
            this.#onPositionChange({ page: startPage, totalPages: this.#totalPages });
        }
    ...
    #updateCurrentPageFromScroll() {
        ...
        if (closestPage !== this.#currentPage) {
            ...
            if (this.#onPositionChange) {
                this.#onPositionChange({ page: this.#currentPage, totalPages: this.#totalPages });
            }
        }
    }
    ...
    async #renderPage(pageNum, container) {
        try {
            ...
            this.#renderTextLayer(page, viewport, wrapper).catch(err => {
                console.warn(`[PDF] Text layer failed for page ${pageNum}:`, err.message);
            });
    ...
    goToPage(pageNum) {
        ...
        const pageContainer = this.#container.querySelector(`.pdf-page-container[data-page="${pageNum}"]`);
        if (pageContainer) {
    ...
    async searchText(query, options = {}) {
        ...
        for (let pageNum = 1; pageNum <= this.#totalPages; pageNum++) {
            ...
            while (true) {
                ...
                if (wholeWord) {
                    ...
                    const charAfter = index + query.length < pageText.length ? pageText[index + query.length] : ' ';
                ...
                const consistency = validate ? this.#validateMatch(matchedText, query, pageNum, matches.length + 1) : { valid: true };
                
                // Skip invalid matches - they don't actually contain the query
                if (!consistency.valid) {
                    if (debug) {
                        console.warn(`[PDFViewer] Skipping invalid match on page ${pageNum}:`, consistency.issues);
                    }
                    startIndex = index + 1;
                    continue;
                }
        ...
        if (debug) {
            ...
            console.log(`[PDFViewer] Found ${totalMatches} matches in ${results.length} pages`);
        }
    ...
    #validateMatch(matchedText, query, pageNum, matchIndex) {
        ...
        if (Math.abs(matchedText.length - query.length) > 2) {
            issues.push(`Length mismatch: query=${query.length}, matched=${matchedText.length}`);
        }
        
        // Check 3: Character similarity
        const similarity = this.#calculateSimilarity(matchedText.toLowerCase(), query.toLowerCase());
        if (similarity < 0.8) {
    ...
    highlightSearchResults(results, query) {
        console.log(`[PDFViewer] Highlighting ${results.length} pages for query: "${query}"`);
        ...
        results.forEach(result => {
            const container = this.#container.querySelector(`.pdf-page-container[data-page="${result.page}"]`);
            if (!container) {
            ...
            console.log(`[PDFViewer] Page ${result.page} textLayer has ${textLayer.querySelectorAll('span').length} spans`);
            
            result.matches.forEach((match, matchIdx) => {
                // Handle match object structure - match.text might contain validation object
                const searchText = typeof match.text === 'string' ? match.text : 
                                  (match.text && match.text.text ? match.text.text : String(match.text));
                
                if (!searchText || searchText.trim() === '') {
                    console.warn(`[PDFViewer] Empty match text on page ${result.page}, match ${matchIdx}`);
                    return;
                }
                
                console.log(`[PDFViewer] Looking for: "${searchText}" on page ${result.page}`);
                ...
                textSpans.forEach(span => {
                    ...
                    if (span.textContent && span.textContent.toLowerCase().includes(searchText.toLowerCase())) {
                        const originalText = span.textContent;
                        console.log(`[PDFViewer] Found match in span: "${originalText.substring(0, 50)}..."`);
                        ...
                        if (newHTML !== originalText) {
                            ...
                            found = true;
                            totalHighlighted++;
                            console.log(`[PDFViewer] Highlighted match #${totalHighlighted} on page ${result.page}`);
                        }
                    }
                });
                
                if (!found) {
                    console.warn(`[PDFViewer] Could not find "${searchText}" in textLayer on page ${result.page}`);
                }
            });
        });
    ...
    async printPdf() {
        ...
        try {
            ...
            for (let pageNum = 1; pageNum <= this.#totalPages; pageNum++) {
                const page = await this.#pdf.getPage(pageNum);
                const viewport = page.getViewport({ scale: 1.5, rotation: this.#rotation || 0 }); // Higher scale for printing
                ...
                img.style.cssText = 'display: block; margin: 10px 0; page-break-after: always;';
...
class BookReaderApp {
    ...
    async #loadPdf(book) {
        ...
        await new Promise(requestAnimationFrame);
        const metadata = await this.#pdfViewer.loadBook(book.data, book.id, startPage);
    ...
    #updatePdfIndicator(position) {
        ...
        if (this.#currentBook && this.#currentBook.id) {
            ...
            this._saveTimeout = setTimeout(() => {
                this.#library?.updateBookPosition(this.#currentBook.id, { page: position.page });
            }, 1000);
        }
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (location.protocol === 'file:' && !window.electronAPI?.isElectron) {
        ...
        if (errorDiv) {
            ...
            if (descEl) descEl.innerHTML = 'Bu uygulama file:// protokolu ile calismaz. HTTP sunucusu kullanin.';
        }
        return;
    }
...
window.debugEPUB = () => {
    ...
    console.log('PDF Viewer:', !!(window.bookReaderApp && window.bookReaderApp.getPdfViewer));
    console.log('Library:', !!(window.bookReaderApp && window.bookReaderApp.getLibrary));
    console.log('=============');
};
